...
import math, pickle
from Old import Chunk, gameUtilities, items, tiles
...
class Projectile:
    def __init__(self, id:int, pos:list, entityBuffer:EntityBuffer, width:int, height:int, hitboxWidth:int, hitboxHeight:int, hitboxOff:tuple=(0,0)):
        self.id = id
        self.pos = pos
        ...
        self.hitboxOff = hitboxOff

        self.hitboxPos = [self.pos[0] + self.hitboxOff[0], self.pos[1] + self.hitboxOff[1]]
        self.surf = pygame.Surface((self.width, self.height))
        self.surfPos = lambda: [ self.pos[0] - (self.width//2), self.pos[1] + (self.height//2) ]
        pass
